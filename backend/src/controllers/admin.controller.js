...
const getDoctors = asyncHandler(async (req, res) => {
  ...
  const q = { role:"DOCTOR" };
  if (search) { const r = new RegExp(search,"i"); q.$or = [{ firstName:r },{ lastName:r },{ email:r }]; }
  const [users, total] = await Promise.all([User.find(q).skip(skip).limit(Number(limit)).lean(), User.countDocuments(q)]);
...
const getPatients = asyncHandler(async (req, res) => {
  ...
  if (search) { const r = new RegExp(search,"i"); q.$or=[{firstName:r},{lastName:r},{email:r}]; }
  const [patients,total] = await Promise.all([User.find(q).skip(skip).limit(Number(limit)).lean(), User.countDocuments(q)]);
